    ...
    tabBind:function () {
        ...
        $("#report2").on({
            'click':function () {
                ...
                var endTime = data.split(" - ")[1];
                var type = "1";
                location.href = property.getProjectPath()+"postLiterature/exportLiteratureReport2.do?startTime="+startTime+
                    "&endTime="+endTime+"&type="+type;
                return false;
            }
        })
...
function getReport2(data) {
    ...
    $.ajax({
        ...
        type:"post",
        async:false,
        url:property.getProjectPath()+"postLiterature/getLiteratureReport2.do",
        success:function(result) {
            if (result.success == 1) {
                var data = result.data;
                barData = data.bar;
...
function loadChart1() {
    layui.use('table', function(){
        ...
        table.render({
            ...
            ,cols: [[
                {field:'type', title:'类型'}
                ,{field:'ZHIZHI', title:'纸质版本(次)'}
                ,{field:'DIANZI', title:'电子版本(次)'}
                ,{field:'DANGAN', title:'档案版本(次)'}
            ]]
        });
    });
    main.initPie();
