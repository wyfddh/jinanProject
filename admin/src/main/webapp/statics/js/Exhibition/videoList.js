    ...
    layui.use('table', function(){
        ...
        table.render({
            ...
            ,cols: [[
                ...
                ,{field:'planTime', title:'策划时间', align:"center", sort: true,
            ...
            switch(obj.event){
                ...
                case 'getCheckData':
                    ...
                    if (idsArray.length == 0) {
                        top.layer.msg('请选择多选框');
                        return ;
                    } else {
                        // reduce
                        parent.layer.confirm('确定要删除么',{icon:3, title:'删除确认'}, function(index){
                            var ids = idsArray.join(',');
                            yc.ajaxGetByParams('exhib/updateExhibByIds.do', {'ids': ids}, '批量删除成功');
                            layer.close(index);
                            that.initTable(null);
                        });
                    }
        ...
        table.on('tool(test)', function(obj){
            ...
            if(obj.event === 'del'){
                parent.layer.confirm('确定要删除么',{icon:3, title:'删除确认'}, function(index){
                    ...
                    yc.ajaxGetByParams('exhib/updateExhibByIds.do', {'ids': delId}, '删除成功');
                    layer.close(index);
                    // _this.initTable(JSON.stringify(json))
                    that.initTable(null);
                });
