...
(function ($, window) {
    ...
    window.$t.goToPage=function(obj,back,callback) {
        var $this = $(obj);
        var name = $this.attr("title");
        var url = $this.attr("data-url")|| "";
        //判断该页面是否已存在
        if (parent.$("#navTab").find("li[data-url='" + url + "']").length === 0) {

            //如果不存在
            //$("#navTab").find("li").removeClass("layui-this");
            //新增tab页
            var _li = '<li class="layui-this main-tab" style="width:100%" data-url="' + url + '">'+"<span style='float:left'>"+name+"</span>"+"<span style='float:right' class='myRefresh'><i class=\"fa fa-refresh\">&nbsp;</i>返回</span>"+ '</li>';
            parent.$("#navTab").find("ul").empty().append(_li);

            //$("#main").find("iframe").removeClass("cy-show");
            //打开iframe
            var iframe = $('<iframe class="cy-show" scrolling="yes" frameborder="0" ></iframe>');
            $(iframe).attr("src", url);
            parent.$("#main").empty().append(iframe);
    ...
    window.$t.goToPageSimple=function(url,name,back) {
        //判断该页面是否已存在
        if (parent.$("#navTab").find("li[data-url='" + url + "']").length === 0) {
    ...
    window.$t.goback=function(url){
        if (url == 'main/comprehensiveQuery.html') {
            $('#startSearch').click();
        } else {
            $.each($(".layui-nav-tree a"), function (i, elm) {
                if ($(elm).attr("data-url") == url) {
                    $(elm).click();
                }
            })
        }
    ...
    window.$t.getQueryStringFrame = function(name) {
        var reg = new RegExp('(^|&)' + name + '=([^&]*)(&|$)', 'i');
        var urlArr = $(parent.$("#main iframe")).attr('src').split('?');
        var r;
        if(urlArr.length > 1) {
