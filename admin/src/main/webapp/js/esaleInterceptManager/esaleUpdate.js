...
layui.use(['form', 'layer', 'layedit', 'laydate'], function() {
    ...
    form.on("submit(updateInterceptor)", function (data) {
      /*  var indexLoading = layer.open({
        ...
        $.ajax({
            type: "post",
            data: formData,
            url: projectName + 'interceptKeyWord/updateInterceptWords.do?',
    ...
    initTable:function(){
        ...
        layui.use('form', function(){
          ...
          /*  form.on('submit(updateProvider)', function(data){
                ...
                //使用ajax提交
                $.ajax({
                    url:'${pageContext.request.contextPath }/provider/updateProvider.action',
                    type:'POST',
...
function loadData(id) {
    layui.use('form', function(){
        ...
        $.ajax({
            ...
            data:json,
            async:false,
            url:property.getProjectPath()+"interceptKeyWord/toUpdate.do",
            success:function(result) {
                /*  if (result.success == 1) {*/
                setFormData(result.data);
                form.render('select');
...
function setFormData(data) {
    ...
    $("#id").val(data.id);
    $("#interceptName").val(data.interceptName);
    $("#interceptNum").val(data.interceptNum);
...
form.on('submit(updateProvider)', function(data){
    ...
    //使用ajax提交
    $.ajax({
        url:property.getProjectPath()+'/interceptKeyWord/updateInterceptWords.do',
        type:'POST',
        async:true,    //或false,是否异步
        data:data,
        timeout:5000,    //超时时间
