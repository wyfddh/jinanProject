...
function loadTable(){
    layui.use('table', function(){
        ...
        table.render({
            elem: '#test'
            ,url:property.getProjectPath()+"curator/getCuratorList.do?userName="+userName+"&processState="+processState+"&processResult="+processResult
            ,request:{
                ...
                layer.confirm('真的删除行么', function(index){
                    ...
                    $.ajax({
                        ...
                        success:function(result) {
                            if (result.success == 1) {
                                top.layer.msg("删除公众策展成功");
                                loadTable();
                            } else {
                                //top.layer.msg(result.error.message);
                                top.layer.msg("删除公众策展失败");
                            }
                        },
