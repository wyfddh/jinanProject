...
layui.define(["element","jquery"],function(exports){
	...
	Tab.prototype.render = function() {
        ...
        $("body .layui-nav-tree .layui-nav-item a")[0].click();
        var selecteItem = $("body .layui-nav-tree .layui-nav-item .layui-nav-child a")[0];
		debugger
        $("#upTitle").text($(selecteItem).find("cite").text());
        $("#upIframe").attr("src",selecteItem.dataset.url);
	}
	...
	$(".closePageOther").on("click",function(){
		if($("#top_tabs li").length>2 && $("#top_tabs li.layui-this cite").text()!="博物馆管理"){
			var menu = JSON.parse(window.sessionStorage.getItem("menu"));
		...
		}else if($("#top_tabs li.layui-this cite").text()=="博物馆管理" && $("#top_tabs li").length>1){
			$("#top_tabs li").each(function(){
				if($(this).attr("lay-id") != '' && !$(this).hasClass("layui-this")){
					element.tabDelete("bodyTab",$(this).attr("lay-id")).init();
